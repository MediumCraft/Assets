...
const { composed: PriceIndication_composed } = (external_highcharts_src_js_default_default());

const { addEvent: PriceIndication_addEvent, merge: PriceIndication_merge, pushUnique: PriceIndication_pushUnique } = (external_highcharts_src_js_default_default());
/* *
 *
 *  Composition
 *
 * */
/** @private */
function PriceIndication_compose(SeriesClass) {
    if (PriceIndication_pushUnique(PriceIndication_composed, 'PriceIndication')) {
        PriceIndication_addEvent(SeriesClass, 'afterRender', onSeriesAfterRender);
        PriceIndication_addEvent(SeriesClass, 'hide', onSeriesHide);
    }
}
/**
...
function onSeriesAfterRender() {
    ...
    if ((lastVisiblePrice || lastPrice) &&
        ...
        if (lastVisiblePrice && lastVisiblePrice.enabled && pLength > 0) {
            yAxis.crosshair = yAxis.options.crosshair = PriceIndication_merge({
...
const PriceIndication = {
    compose: PriceIndication_compose
};
...
const price_indicator_src_G = (external_highcharts_src_js_default_default());
Extensions_PriceIndication.compose(price_indicator_src_G.Series);
