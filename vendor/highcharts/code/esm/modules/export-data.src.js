 ...
 * @optionparent exporting
 */
const ExportingDefaults_exporting = {
    /**
...
const ExportingDefaults = {
    exporting: ExportingDefaults_exporting,
    lang: ExportingDefaults_lang,
    navigation
};
...
const { composed: Fullscreen_composed } = (external_highcharts_src_js_default_default());

const { addEvent: Fullscreen_addEvent, fireEvent: Fullscreen_fireEvent, pushUnique: Fullscreen_pushUnique } = (external_highcharts_src_js_default_default());
/* *
 *
 *  Functions
 *
...
class Fullscreen {
    ...
    static compose(ChartClass) {
        if (Fullscreen_pushUnique(Fullscreen_composed, 'Fullscreen')) {
            // Initialize fullscreen
            Fullscreen_addEvent(ChartClass, 'beforeRender', onChartBeforeRender);
        }
    }
    /* *
     *
    ...
    close() {
        ...
        Fullscreen_fireEvent(chart, 'fullscreenClose', null, function () {
    ...
    open() {
        const fullscreen = this, chart = fullscreen.chart, optionsChart = chart.options.chart;
        Fullscreen_fireEvent(chart, 'fullscreenOpen', null, function () {
            if (optionsChart) {
            ...
            if (fullscreen.browserProps) {
                const unbindChange = Fullscreen_addEvent(chart.container.ownerDocument, // Chart's document
                ...
                const unbindDestroy = Fullscreen_addEvent(chart, 'destroy', unbindChange);
...
const { defaultOptions, setOptions: Exporting_setOptions } = (external_highcharts_src_js_default_default());
...
const { addEvent: Exporting_addEvent, clearTimeout: Exporting_clearTimeout, createElement, css, discardElement, error: Exporting_error, extend: Exporting_extend, find: Exporting_find, fireEvent: Exporting_fireEvent, isObject, merge, objectEach, pick: Exporting_pick, pushUnique: Exporting_pushUnique, removeEvent, splat, uniqueKey } = (external_highcharts_src_js_default_default());
...
class Exporting {
    ...
    addButton(options) {
        ...
        if (btnOptions.text && btnOptions.symbol) {
            theme.paddingLeft = Exporting_pick(theme.paddingLeft, 30);
        }
        else if (!btnOptions.text) {
            Exporting_extend(theme, {
        ...
        const button = renderer
            ...
            .attr({
            title: Exporting_pick(chart.options.lang[(btnOptions._titleKey ||
                btnOptions.titleKey)], '')
        });
        ...
        button
            .add(exporting.group)
            .align(Exporting_extend(btnOptions, {
            width: button.width,
            x: Exporting_pick(btnOptions.x, exporting.buttonOffset) // #1654
        }), true, 'spacingBox');
    ...
    contextMenu(className, items, x, y, width, height, button) {
        ...
        if (!menu) {
            ...
            // Presentational CSS
            if (!chart.styledMode) {
                css(innerMenu, Exporting_extend({
    ...
    async exportChart(exportingOptions, chartOptions) {
        ...
        if (exportingOptions.local) {
            ...
            if (exportingOptions.url) {
                await external_data_src_js_default_HttpUtilities_default().post(exportingOptions.url, {
    ...
    getSVG(chartOptions) {
        ...
        Exporting_extend(options.chart, {
            animation: false,
            renderTo: sandbox,
            forExport: true,
        ...
        chart.axes.forEach(function (axis) {
            const axisCopy = Exporting_find(chartCopy.axes, (copy) => copy.options.internalKey === axis.userOptions.internalKey);
            if (axisCopy) {
    ...
    async localExport(exportingOptions, chartOptions) {
        ...
        try {
            ...
            if (sanitizedSVG.indexOf('<foreignObject') > -1 &&
                ...
                await exporting.downloadSVG(sanitizedSVG, Exporting_extend({ filename: exporting.getFilename() }, exportingOptions));
            }
...
(function (Exporting) {
    ...
    function compose(ChartClass, SVGRendererClass) {
        ...
        // Adding wrappers for the deprecated functions
        Exporting_extend((external_highcharts_src_js_default_Chart_default()).prototype, {
        ...
        Exporting_setOptions(Exporting_ExportingDefaults);
    }
...
// Compatibility
G.HttpUtilities = G.HttpUtilities || (external_data_src_js_default_HttpUtilities_default());
...
/* harmony default export */ const exporting_src = ((/* unused pure expression or super */ null && (Highcharts)));
...
const export_data_src_G = (external_highcharts_src_js_default_default());
// Compatibility
export_data_src_G.dataURLtoBlob = export_data_src_G.dataURLtoBlob || Shared_DownloadURL.dataURLtoBlob;
export_data_src_G.downloadURL = export_data_src_G.downloadURL || Shared_DownloadURL.downloadURL;
// Compose
ExportData_ExportData.compose(export_data_src_G.Chart, export_data_src_G.Exporting, export_data_src_G.Series);
