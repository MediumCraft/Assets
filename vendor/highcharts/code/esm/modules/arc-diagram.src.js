...
const { seriesTypes: { sankey: { prototype: { pointClass: ArcDiagramPoint_SankeyPoint } } } } = (external_highcharts_src_js_default_SeriesRegistry_default());
 ...
 *
 * */
class ArcDiagramPoint extends ArcDiagramPoint_SankeyPoint {
    /* *
     *
     *  Functions
     *
...
const { seriesTypes: { column: ArcDiagramSeries_ColumnSeries, sankey: ArcDiagramSeries_SankeySeries } } = (external_highcharts_src_js_default_SeriesRegistry_default());
const { crisp: ArcDiagramSeries_crisp, extend: ArcDiagramSeries_extend, merge: ArcDiagramSeries_merge, pick: ArcDiagramSeries_pick, relativeLength: ArcDiagramSeries_relativeLength } = (external_highcharts_src_js_default_default());
/* *
 *
 *  Class
 *
 ...
 */
class ArcDiagramSeries extends ArcDiagramSeries_SankeySeries {
    /* *
     *
     *  Functions
     *
    ...
    translateNode(node, column) {
        const series = this, translationFactor = series.translationFactor, chart = series.chart, maxNodesLength = chart.inverted ?
            ...
            maxRadius :
            Math.max(sum * translationFactor, this.options.minLinkWidth || 0), lineWidth = options.marker?.lineWidth || 0, nodeOffset = column.sankeyColumn.offset(node, translationFactor), fromNodeLeft = ArcDiagramSeries_crisp(ArcDiagramSeries_pick(nodeOffset && nodeOffset.absoluteLeft, ((column.sankeyColumn.left(translationFactor) || 0) +
            ...
            ((chart.inverted ?
                chart.plotWidth : chart.plotHeight) - (ArcDiagramSeries_crisp(this.colDistance * (node.column || 0) +
        ...
        if (sum) {
            ...
            if (this.mapOptionsToLevel) {
                // Calculate data label options for the point
                node.dlOptions = {
                    ...ArcDiagramSeries_SankeySeries.getDLOptions({
                        level: this.mapOptionsToLevel[node.level],
                        optionsPoint: node.options
                    }),
                    zIndex: void 0
    ...
    drawDataLabels() {
        if (this.options.dataLabels) {
            ...
            // Render node labels:
            ArcDiagramSeries_ColumnSeries.prototype.drawDataLabels.call(this, this.nodes);
            // Render link labels:
            this.options.dataLabels.textPath =
                this.options.dataLabels.linkTextPath;
            ArcDiagramSeries_ColumnSeries.prototype.drawDataLabels.call(this, this.data);
            // Restore nodes
 ...
 *
 * */
ArcDiagramSeries.defaultOptions = ArcDiagramSeries_merge(ArcDiagramSeries_SankeySeries.defaultOptions, ArcDiagram_ArcDiagramSeriesDefaults);
