...
/* harmony default export */ const sankey_src = ((/* unused pure expression or super */ null && (Highcharts)));
...
const { defined: OrganizationPoint_defined, find: OrganizationPoint_find, pick: OrganizationPoint_pick } = (external_highcharts_src_js_default_default());
/* *
 *
 *  Functions
 *
...
class OrganizationPoint extends SankeyPointClass {
    ...
    setNodeColumn() {
        ...
        if (
        // Not defined by user
        !OrganizationPoint_defined(node.options.column) &&
            // Has links to
            node.linksTo.length !== 0 &&
            ...
            node.options.layout = OrganizationPoint_pick(node.options.layout, 'hanging');
            node.hangsFrom = fromNode;
            OrganizationPoint_find(fromNode.linksFrom, (link, index) => {
...
const { sankey: OrganizationSeries_SankeySeries } = (external_highcharts_src_js_default_SeriesRegistry_default()).seriesTypes;

const { css, crisp: OrganizationSeries_crisp, extend: OrganizationSeries_extend, isNumber: OrganizationSeries_isNumber, merge: OrganizationSeries_merge, pick: OrganizationSeries_pick } = (external_highcharts_src_js_default_default());
 ...
 */
class OrganizationSeries extends OrganizationSeries_SankeySeries {
    /* *
     *
     *  Functions
     *
    ...
    pointAttribs(point, state) {
        const series = this, attribs = OrganizationSeries_SankeySeries.prototype.pointAttribs.call(series, point, state), level = point.isNode ? point.level : point.fromNode.level, levelOptions = series.mapOptionsToLevel[level || 0] || {}, options = point.options, stateOptions = (levelOptions.states &&
        ...
        if (!point.isNode) {
            ...
            if (OrganizationSeries_isNumber(borderRadius)) {
    ...
    translateLink(point) {
        ...
        if (fromNode.shapeArgs && toNode.shapeArgs) {
            ...
            let x1 = OrganizationSeries_crisp((fromNode.shapeArgs.x || 0) +
                (fromNode.shapeArgs.width || 0), linkWidth), y1 = OrganizationSeries_crisp((fromNode.shapeArgs.y || 0) +
                (fromNode.shapeArgs.height || 0) / 2, linkWidth), x2 = OrganizationSeries_crisp(toNode.shapeArgs.x || 0, linkWidth), y2 = OrganizationSeries_crisp((toNode.shapeArgs.y || 0) +
            ...
            if (toNode.hangsFrom === fromNode) {
                if (chart.inverted) {
                    y1 = !hangingRight ?
                        OrganizationSeries_crisp((fromNode.shapeArgs.y || 0) +
                            (fromNode.shapeArgs.height || 0) -
                            hangingIndent / 2, linkWidth) :
                        OrganizationSeries_crisp((fromNode.shapeArgs.y || 0) + hangingIndent / 2, linkWidth);
                    y2 = !hangingRight ? ((toNode.shapeArgs.y || 0) +
                        (toNode.shapeArgs.height || 0)) : (toNode.shapeArgs.y || 0) + hangingIndent / 2;
                }
                else {
                    y1 = OrganizationSeries_crisp((fromNode.shapeArgs.y || 0) + hangingIndent / 2, linkWidth);
                }
                xMiddle = x2 = OrganizationSeries_crisp((toNode.shapeArgs.x || 0) +
 ...
 *
 * */
OrganizationSeries.defaultOptions = OrganizationSeries_merge(OrganizationSeries_SankeySeries.defaultOptions, Organization_OrganizationSeriesDefaults);
