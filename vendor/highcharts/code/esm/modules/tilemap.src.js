...
;// external ["./coloraxis.src.js","default","ColorAxis"]
const external_coloraxis_src_js_default_ColorAxis_namespaceObject = __WEBPACK_EXTERNAL_MODULE__coloraxis_src_js_cdd22a72__["default"].ColorAxis;
...
G.ColorAxis = G.ColorAxis || (external_coloraxis_src_js_default_ColorAxis_default());
...
MapPoint_extend(MapPoint.prototype, {
    dataLabelOnNull: (external_coloraxis_src_js_default_ColorMapComposition_default()).pointMembers.dataLabelOnNull,
    moveToTopOnHover: (external_coloraxis_src_js_default_ColorMapComposition_default()).pointMembers.moveToTopOnHover,
    isValid: (external_coloraxis_src_js_default_ColorMapComposition_default()).pointMembers.isValid
});
/* *
 *
 *  Default Export
...
MapSeries_extend(MapSeries.prototype, {
    ...
    colorKey: (external_coloraxis_src_js_default_ColorMapComposition_default()).seriesMembers.colorKey,
    ...
    getSymbol: noop,
    isCartesian: false,
    parallelArrays: (external_coloraxis_src_js_default_ColorMapComposition_default()).seriesMembers.parallelArrays,
    pointArrayMap: (external_coloraxis_src_js_default_ColorMapComposition_default()).seriesMembers.pointArrayMap,
    pointClass: Map_MapPoint,
    // X axis and Y axis must have same translation slope
    preserveAspectRatio: true,
    searchPoint: noop,
    trackerGroups: (external_coloraxis_src_js_default_ColorMapComposition_default()).seriesMembers.trackerGroups,
    // Get axis extremes from paths, not values
    useMapGeometry: true
});
external_coloraxis_src_js_default_ColorMapComposition_default().compose(MapSeries);
...
const { series: Series, seriesTypes: { column: HeatmapSeries_ColumnSeries, scatter: HeatmapSeries_ScatterSeries } } = (external_highcharts_src_js_default_SeriesRegistry_default());
...
external_coloraxis_src_js_default_ColorMapComposition_default().compose(HeatmapSeries);
...
// Classes
map_src_G.ColorMapComposition = (external_coloraxis_src_js_default_ColorMapComposition_default());
...
const { addEvent: ColorAxisComposition_addEvent, extend: ColorAxisComposition_extend, merge: ColorAxisComposition_merge, pick: ColorAxisComposition_pick, splat: ColorAxisComposition_splat } = (external_highcharts_src_js_default_default());
/* *
 *
 *  Composition
 *
...
const { series: { prototype: { pointClass: Point } }, seriesTypes: { heatmap: { prototype: { pointClass: TilemapPoint_HeatmapPoint } } } } = (external_highcharts_src_js_default_SeriesRegistry_default());
 ...
 *
 * */
class TilemapPoint extends TilemapPoint_HeatmapPoint {
    /* *
     *
     *  Functions
     *
...
const { noop: TilemapShapes_noop } = (external_highcharts_src_js_default_default());

const { heatmap: TilemapShapes_HeatmapSeries, scatter: TilemapShapes_ScatterSeries } = (external_highcharts_src_js_default_SeriesRegistry_default()).seriesTypes;

const { clamp: TilemapShapes_clamp, pick: TilemapShapes_pick } = (external_highcharts_src_js_default_default());
/* *
 *
 *  Functions
 *
...
const TilemapShapes = {
    // Hexagon shape type.
    hexagon: {
        alignDataLabel: TilemapShapes_ScatterSeries.prototype.alignDataLabel,
        ...
        translate: function () {
            ...
            series.generatePoints();
            for (const point of series.points) {
                let x1 = TilemapShapes_clamp(Math.floor(xAxis.len -
                    xAxis.translate(point.x - xPad * 2, 0, 1, 0, 1)), -xAxis.len, 2 * xAxis.len), x2 = TilemapShapes_clamp(Math.floor(xAxis.len -
                    xAxis.translate(point.x - xPad, 0, 1, 0, 1)), -xAxis.len, 2 * xAxis.len), x3 = TilemapShapes_clamp(Math.floor(xAxis.len -
                    xAxis.translate(point.x + xPad, 0, 1, 0, 1)), -xAxis.len, 2 * xAxis.len), x4 = TilemapShapes_clamp(Math.floor(xAxis.len -
                    xAxis.translate(point.x + xPad * 2, 0, 1, 0, 1)), -xAxis.len, 2 * xAxis.len), y1 = TilemapShapes_clamp(Math.floor(yAxis.translate(point.y - yPad, 0, 1, 0, 1)), -yAxis.len, 2 * yAxis.len), y2 = TilemapShapes_clamp(Math.floor(yAxis.translate(point.y, 0, 1, 0, 1)), -yAxis.len, 2 * yAxis.len), y3 = TilemapShapes_clamp(Math.floor(yAxis.translate(point.y + yPad, 0, 1, 0, 1)), -yAxis.len, 2 * yAxis.len);
    ...
    diamond: {
        ...
        translate: function () {
            ...
            series.generatePoints();
            for (const point of series.points) {
                let x1 = TilemapShapes_clamp(Math.round(xAxis.len -
                    xAxis.translate(point.x - xPad, 0, 1, 0, 0)), -xAxis.len, 2 * xAxis.len), x3 = TilemapShapes_clamp(Math.round(xAxis.len -
                    xAxis.translate(point.x + xPad, 0, 1, 0, 0)), -xAxis.len, 2 * xAxis.len), y1 = TilemapShapes_clamp(Math.round(yAxis.translate(point.y - yPad, 0, 1, 0, 0)), -yAxis.len, 2 * yAxis.len), y2 = TilemapShapes_clamp(Math.round(yAxis.translate(point.y, 0, 1, 0, 0)), -yAxis.len, 2 * yAxis.len), y3 = TilemapShapes_clamp(Math.round(yAxis.translate(point.y + yPad, 0, 1, 0, 0)), -yAxis.len, 2 * yAxis.len);
                const x2 = TilemapShapes_clamp(Math.round(xAxis.len -
    ...
    circle: {
        ...
        haloPath: function (size) {
            return TilemapShapes_ScatterSeries.prototype.pointClass.prototype.haloPath
        ...
        translate: function () {
            ...
            series.generatePoints();
            for (const point of series.points) {
                const x = TilemapShapes_clamp(Math.round(xAxis.len -
                    xAxis.translate(point.x, 0, 1, 0, 0)), -xAxis.len, 2 * xAxis.len);
                let pointPadding = seriesPointPadding, hasPerPointPadding = false, y = TilemapShapes_clamp(Math.round(yAxis.translate(point.y, 0, 1, 0, 0)), -yAxis.len, 2 * yAxis.len);
                ...
                */
                if (!radius || forceNextRadiusCompute) {
                    colsizePx = Math.abs(TilemapShapes_clamp(Math.floor(xAxis.len -
                        xAxis.translate(point.x + colsize, 0, 1, 0, 0)), -xAxis.len, 2 * xAxis.len) - x);
                    yRadiusPx = Math.abs(TilemapShapes_clamp(Math.floor(yAxis.translate(point.y + yRadius, 0, 1, 0, 0)), -yAxis.len, 2 * yAxis.len) - y);
    ...
    // Square shape type.
    square: {
        alignDataLabel: TilemapShapes_HeatmapSeries.prototype.alignDataLabel,
        translate: TilemapShapes_HeatmapSeries.prototype.translate,
        getSeriesPadding: TilemapShapes_noop,
        haloPath: TilemapShapes_HeatmapSeries.prototype.pointClass.prototype.haloPath
    }
};
/* *
 *
...
const { composed: TilemapSeries_composed, noop: TilemapSeries_noop } = (external_highcharts_src_js_default_default());

const { column: TilemapSeries_ColumnSeries, heatmap: TilemapSeries_HeatmapSeries, scatter: TilemapSeries_ScatterSeries } = (external_highcharts_src_js_default_SeriesRegistry_default()).seriesTypes;




const { addEvent: TilemapSeries_addEvent, extend: TilemapSeries_extend, merge: TilemapSeries_merge, pushUnique: TilemapSeries_pushUnique } = (external_highcharts_src_js_default_default());
/* *
 *
 *  Functions
 *
...
class TilemapSeries extends TilemapSeries_HeatmapSeries {
    ...
    static compose(AxisClass) {
        if (TilemapSeries_pushUnique(TilemapSeries_composed, 'TilemapSeries')) {
    ...
    drawPoints() {
        ...
        TilemapSeries_ColumnSeries.prototype.drawPoints.call(this);
...
TilemapSeries_extend(TilemapSeries.prototype, {
    ...
    pointAttribs: TilemapSeries_ColumnSeries.prototype.pointAttribs,
...
const tilemap_src_G = (external_highcharts_src_js_default_default());
Tilemap_TilemapSeries.compose(tilemap_src_G.Axis);
