/**
 *
 *  (c) 2010-2025 Pawel Fus & Daniel Studencki
 *
 *  License: www.highcharts.com/license
 *
 *  !!!!!!! SOURCE GETS TRANSPILED BY TYPESCRIPT. EDIT TS FILE ONLY. !!!!!!!
 *
 * */
/**
 * Get extremes of array filled by OHLC data.
 *
 * @private
 *
 * @param {Array<Array<number>>} arr
 * Array of OHLC points (arrays).
 *
 * @param {number} minIndex
 * Index of "low" value in point array.
 *
 * @param {number} maxIndex
 * Index of "high" value in point array.
 *
 * @return {Array<number,number>}
 * Returns array with min and max value.
 */
declare function getArrayExtremes<T extends Array<number>>(arr: Array<T>, minIndex: number, maxIndex: number): ArrayUtilities.Extremes;
declare namespace ArrayUtilities {
    /** 0: min, 1: max */
    type Extremes = [number, number];
}
declare const ArrayUtilities: {
    getArrayExtremes: typeof getArrayExtremes;
};
export default ArrayUtilities;
